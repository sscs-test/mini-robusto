...
## Changes required to run for any repository

./owner_alice/create_layout.py file -> change it to work with your repository
(pastebin-app)

Use your own keys to sign the packaging


The basepath of the target file is of the mirror but in this case it is the path
to this repository in my system.

You wanted me to use one key hence used not using threshold 2 for root role
signing.

The in-toto verify also creates an untar link file. Should that also be
distributed using TUF.
