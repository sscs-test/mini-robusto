...
signers[delegatee_name] = timestamp_snapshot_key
...
# del roles["targets"].signed.targets[alice_pub_target] # no need to remove
# del roles["targets"].signed.targets[layout_target] # no need to remove
...
for role_name in ["targets", "packages-and-in-toto-metadata-signer", "snapshot", "timestamp"]:
